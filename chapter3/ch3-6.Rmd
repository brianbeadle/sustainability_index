...
This file estimates the relationship between AS and energy and industrial using a non-parametric approach.
...
# subsample <- getdata(d, samplecluster)
# glimpse(subsample, width = 60)
```
...
fit5 <- brm( 
  ...
  warmup = 500,
  refresh = 0,
  file = "../results/130-2pl-spline-X.rds"
)
toc()
```
...
```{r}
# conditional_effects(fit5,  effects = c("industrial:lfa"), categorical = TRUE, prob = 0.8)
```


```{r}
conditional_effects(fit5,  effects = c("industrial:lfa"), prob = 0.8,
...
```{r}
conditional_effects(fit5,  effects = c("energy:lfa"), prob = 0.8,
                    dpar = "mu", method = "posterior_linpred")
```
