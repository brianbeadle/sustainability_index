...
NOTE: ALL DATA USED HERE ARE TIME SERIES 2004-2013 (FADN)
*/
...
di "ASSESSMENT OF THE ECONOMIC DIVERSITY INDICATOR" _n ///
   "Var indicates largest contribution of a single product to total output"
describe `products'
*qui log off
...
* the data are tested to check what is included and if everything sums correctly
* there are four points from the tests conducted below: 
	1) permanent crops is a sum of orchard, other perm, forage crops, fallows, 
	   set aside, out of production, woodland 
 	2) set aside and out of production are often the same. calculating to 
	   remove any double counting and merge them into 1 variable
 	3) the difference between set aside and out of production appears to be 
	   equal to fallow
 	4) land used for energy crops and woodland not included in total uaa
*/
gen out_of_production = SE074-SE073
sum out_of_production // no negative numbers so it is assumed this new 
*			variable will distinguish between set aside and out of production
drop SE074

gen fallow_test = SE072-out_of_production
sum fallow_test // test to see if fallow and out of production are duplicates. 
* produced a min of -.0000305 and a max of .0000153; assumed to be rounding 
* errors so out of production will be dropped to avoid double counting
drop out_of_production
drop fallow_test
gen total_land = SE035+SE041+SE042+SE046+SE050+SE055+SE065+SE071+SE072 ///
	+SE073+SE075
gen land_test = total_land-SE025
sum land_test 
gsort -land_test // checking what is not listed in total UAA
gen test2 = (SE042+SE075)-land_test // energy crops and woods not in total uaa. 
* test2 produced a min of -.0100067 and a max of .0100001, 
* assumed to be rounding difference
drop land_test
drop test2
...
* expenditure per ha on fertilizers, pesticides, and feedstuffs
gen input_sum = (SE295+SE300+SE310+SE315)/land_inprod
gen inputs = .  // produces a categorical variable for input intensities
replace inputs = 0 if input_sum > 250
replace inputs = 1 if input_sum <= 250 & input_sum > 80
replace inputs = 2 if input_sum <= 80

* recategorizing organic: partial or transitioning included with organic
gen organic = .  
replace organic = 0 if A32 == 1
replace organic = 1 if A32 == 2 | A32 == 3

gen irrigation = .  // dummy variable denoting if farm has irrigation
replace irrigation = 0 if A40 == 0
replace irrigation = 1 if A40 > 0

* creating ecological value of used land. See paper for bin explanations
gen evul = .
...
* creating total ecological quality of land on the farm: 
* evul + land not being used in production
gen tot_land = SE025+SE075  // total land on farm
gen noprod = SE072+SE073+SE075  // land not in production (100% eco quality)
gen land_quality = ((land_inprod*evul)+noprod)/tot_land
lab var land_quality "Ecological value of land"
drop land_inprod grazing luha input_sum inputs organic ///
irrigation evul tot_land noprod

********************************************************************************
*** MEDIAN WAGE CALCULATIONS

gen p_wage = SE370/SE021 // some outliers. i.e. values are below 2€/hour
...
lab var paid_wage ///
"wage_ratio=p_wage(n3-median FarmWagesPaid)/med_region_n3(n3 AllSectorMedWage)"
*qui log on
di "`=char(12)' Distrib of paid_wage:" ///
"ratio of farm wage and median all-sector-wage (both nuts-3 medians):"
codebook paid_wage
*qui log off  
...
sum prov_employ
lab var prov_employ ///
`"Provision of employment and contract work to t output [€/€]"'
...
*** compute mean of plausible land rent values by each combo of TF14 and A1
*gen lruv1 = lruv if yrseq == 1 // contains only one nonmissing lruv per farm, 
* (avoids unequal weighting farms w different # of participation years 
* differently according to their #)

local lruvUpLimspec 4000 // for plausible land rental price computation: 
* upper Euro-per-ha-and-year limit for inclusion among specialized horticulture, 
* wine, fruits

local lruvUpLimrest 2000 // for plausible land rental price computation: upper
* Euro-per-ha-and-year limit for inclusion among all other farm types

* set farm specific lruv to missinhg where implausible
replace lruv  = . if  (( inlist(TF14,20,35,36) & lruv>=`lruvUpLimspec' ) ///
	| (!inlist(TF14,20,35,36) & lruv>=`lruvUpLimrest'))
	
* mean of (plausible) land rent values by each combination of TF14
bysort TF14: egen lruvTF14 = mean(lruv) 

* mean of (plausible) land rent values by each combination of TF14 and A1
bysort TF14 A1: egen lruvTF14A1 = mean(lruv) 

* use TF14 avg if avg for specific A1 is missing
replace lruvTF14A1 = lruvTF14 if inlist(lruvTF14A1,.,0)
* END OF: compute mean of land rent values by each combo of TF14 and A1

* to determine for which farms to impute land rent unit value: 
* lower Euro-per-ha-and-year limit for inclusion among specialized 
* horticulture, wine, fruits
local lruvLoLimspec 500

* to determine for which farms to impute land rent unit value: 
* lower Euro-per-ha-and-year limit for inclusion among all other farm types
local lruvLoLimrest 200
 
* replace implausible farm specific lruv by means per A1 and TF14
replace lruv = lruvTF14A1 if  ( inlist(TF14,20,35,36) & ///
	((lruv<`lruvLoLimspec' )|(lruv>`lruvUpLimspec' ))) | ///
	(!inlist(TF14,20,35,36) & ((lruv<`lruvLoLimrest' ) | ///
	(lruv>`lruvUpLimrest'))) 
...
*gen pluv1 = pluv if yrseq == 1  // contains only one nonmissing pluv per farm,
* 	(avoids unequal weighting farms w different # of participation years  ///
*	differently according to their #)

* for plausible paid labor wage computation: 
* 	lower Euro-per-AWU-and-year limit for inclusion
local pluvLoLim 6000 

* for plausible paid labor wage computation: 
* 	upper Euro-per-AWU-and-year limit for inclusion
local pluvUpLim 50000
 
gen lruv1_roh  =  pluv
* set farm specific pluv to missinhg where implausible
replace pluv  = . if  ( pluv<`pluvLoLim' ) | (pluv>`pluvUpLim')

* mean of (plausible) paid labor wage by federal state (A1)
bysort A1: egen pluvA1 = mean(pluv)
* END OF: compute mean of (plausible) paid labor wage values by A1

* replace implausible farm specific pluv by means per A1
replace pluv = pluvA1 if ( pluv<`pluvLoLim' ) | (pluv>`pluvUpLim')
 
lab var pluv ///
"pluv: Paid labor Unit Value (SE370 and SE020, outliers replaced by mean per A1)"
...
* multi factor productivity computed as maySE415/totcost`i'
gen productivity  = SE415/totcost
    lab var productivity "productivity=SE415/totcost [€/€])"
drop lruv* pluv* *cost air
*summarize productivity if yrseq==1, detail    

********************************************************************************
* saving files for further analysis in all three thesis papers
********************************************************************************

* for all calculated indicators and all raw variables remaining 
save dat/110-raw_indicators-panel.dta, replace 

* reducing data set to only include basic variables
...
*qui log on
describe
codebook profit solvency e_diverse productivity pesticide ghg_emissions ///
land_quality prov_employ paid_wage 
*qui log off
 

*end // end program logfi

*logfi  // call program logfi (this command only works if it is not in the ///
* 	last line of the do file, i.e. at least one cr/lf character must follow!)
