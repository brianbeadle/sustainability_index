...
title: |
  ...
  | 3 out of 9 items missing for 50 percent of the units
...
We randomly drop 3 out of 9 items in 50% of the farms.

```{r}
# select 30% of the farms
set.seed(134679)
n <- round(length(unique(d$id))*0.5, 0)
samplecluster <- cluster(d, clustername=c("id"), size=n, method="srswor")
...
fit1 <- brm( 
  ...
  warmup = 500,
  refresh = 0,
  file = "../results/102-cs-2013-incomplete-3-50.rds"
)
toc()
```
...
write.csv(df.re, file = "../results/102-asi-missing-3-50.csv")
```
...
d.incomplete <- read.csv("../results/102-asi-missing-3-50.csv")
