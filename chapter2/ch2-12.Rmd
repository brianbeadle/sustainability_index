...
title: |
  ...
  | 3 out of 9 items missing for 10 percent of the units
...
We randomly drop 3 out of 9 items in 10% of the farms.
...
# select 3 different items out of 9 items
...
for (i in 1:nrow(items2drop)) {
  items2drop$dropitem3[i] = sample(
    setdiff(1:9, c(items2drop$dropitem1[i],items2drop$dropitem2[i])),
            1, replace = TRUE)
}
...
d$y <- ifelse( is.na(d$dropitem1), d$y,
               ifelse( d$dropitem1 == d$item, NA,
                       ifelse( d$dropitem2 == d$item, NA, 
                               ifelse( d$dropitem3 == d$item, NA, d$y ))))
...
fit1 <- brm( 
  ...
  warmup = 500,
  refresh = 0,
  file = "../results/100-cs-2013-incomplete-3-10.rds"
)
toc()
```
...
write.csv(df.re, file = "../results/100-asi-missing-3-10.csv")
```
...
d.incomplete <- read.csv("../results/100-asi-missing-3-10.csv")
...
```
```{r}
cor(d.combine$asi.x, d.combine$asi.y, use = "complete.obs")
```
